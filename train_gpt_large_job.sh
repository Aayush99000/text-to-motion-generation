...
#SBATCH --mem=64G
#SBATCH --time=1:00:00
#SBATCH --output=/scratch/katoch.aa/text-to-motion/logs/train_large_%j.out
#SBATCH --error=/scratch/katoch.aa/text-to-motion/logs/train_large_%j.err

# ── Paths ──────────────────────────────────────────────────────────────────────
WORKDIR=/scratch/katoch.aa/text-to-motion
DATA_DIR=$WORKDIR/dataset
CKPT_DIR=$WORKDIR/checkpoints_gpt_large
...
NEXT_JOB=$(sbatch --dependency=afterany:$SLURM_JOB_ID "$WORKDIR/train_gpt_large_job.sh" | awk '{print $4}')
...
# ── Cache HuggingFace model to scratch (avoids re-downloading) ────────────────
